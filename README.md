...
> **별도 설정 없이 바로 작동합니다.** 홈 디렉토리(`~`)가 자동으로 마운트되므로
> Windows, Mac, Linux 어디서든 `.env` 파일 없이 실행 가능합니다.
> `.claude` 경로도 초기 화면에서 자동 감지됩니다.
...
> **참고**: `.claude`가 홈 디렉토리가 아닌 다른 위치에 있는 경우에만 `HOST_HOME` 환경변수를 설정하세요:
> ```bash
> export HOST_HOME=/custom/path    # 홈이 아닌 경로에 .claude가 있을 때만
> ```
